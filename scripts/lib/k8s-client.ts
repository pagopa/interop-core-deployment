...
import type { K8sSecretInfo } from './k8s-types.js';
...
export async function fetchSecretsInventory(
  ...
  try {
    ...
    response.items.forEach((secret: any) => {
      ...
      // Skip Helm release secrets (sh.helm.release.v*) - these are auto-generated by Helm and not user secrets
      if (secretName.startsWith('sh.helm.release.v')) {
        return;
      }

      const keys = Object.keys(secret.data || {});
      const annotations = Object.fromEntries(Object.entries(secret.metadata?.annotations || {}).map(([key, value]) => [key, String(value)]));
      secretMap.set(secretName, { keys, annotations });
    });
